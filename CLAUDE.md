...
- **Platform-specific code goes in `src/automation/platforms/` only for reusable ATS/job-board families** - when a real platform (e.g. LinkedIn, Avature, Workday, SmartRecruiters) needs custom modal handling, login flows, or unique form structures across multiple jobs/tenants, create a dedicated package. Do not create a platform module for a single employer host or one-off company site unless it is clearly the same reusable engine and the generic path is proven insufficient. Generic behavior stays in `detection.py` and `forms/`
...
- **Do not keep large inline browser-side JS in Python files** - multi-line DOM walkers, debug dumps, field-lookup heuristics, and other non-trivial browser logic should live in `src/automation/browser_scripts/` and be loaded through `browser_scripts/loader.py`. If the `evaluate(...)` body is more than a tiny local one-liner, extract it to a named script file.
...
- **Prefer better structure over more retries** — when automation is failing on custom widgets, first improve deterministic DOM extraction, field labeling, option discovery, and post-action state verification. Do not add extra vision rounds or more model "reasoning" until the browser layer exposes the real control state cleanly.
- **Use text LLMs on structured fields, not raw page chaos** — text models are useful for answer inference only after the form is normalized into clean question text plus exact visible options. If labels/options are noisy or missing, fix extraction first instead of asking the model to guess from screenshots or raw DOM noise.
- **Platform modules handle non-standard DOM only for major reusable platforms** — when a major ATS uses custom dropdown/widget components that don't match standard patterns (e.g. Avature's click-to-open divs, Workday shadow DOM), create `platforms/<ats>.py` with a `prefill(page, profile, settings)` function. Register it in `platforms/__init__.py:get_platform_prefill()`. For one-off employer sites, improve the generic `forms/` extraction/fill path first instead of creating a host-specific module.
